...
⚠⚠ This repo had NEITHER half of that guard, found while porting it to
jdatamunch-mcp on 2026-08-29 -- the standing "a setting fixed in one repo of a
suite is fixed in one repo" lesson. All three servers carried the same
`.claude/`-only exclude and only jcm had the tests.

⚠ `tests/` is shipped inside this sdist (89 members), so anything dropped there
is distributed too. jdocmunch carried a 5.9 MB promotional image in `tests/`
that was 87% of its whole source distribution until its 1.123.2. That one was a
TRACKED file, so exclusion rules and untracked-file scans were both blind to it;
`test_no_oversized_member_ships` is the check that would have seen it.

⚠ `uv.lock` is gitignored here, so it is never distributed. It is deliberately
absent from ALLOWED_ROOT_FILES; the staleness test would fail if someone copied
jcm's list wholesale.
...
def _strip_root(members: list[tarfile.TarInfo]) -> list[str]:
    """Drop the `jdatamunch_mcp-<version>/` prefix every member carries."""
    stripped = []
...
ALLOWED_ROOT_FILES = frozenset({
    ...
    "README.md", "CHANGELOG.md", "QUICKSTART.md", "USER-MANUAL.md",
    # Agent-facing
    "CLAUDE.md",
})
...
def test_no_oversized_member_ships(built_sdist):
    ...
    jdocmunch-mcp carried `tests/infographic.png` -- 5.9 MB, referenced by
    nothing -- as 87% of ITS source distribution until its 1.123.2. It was a
    TRACKED file, so exclusion rules and untracked-file scans were both blind
    to it, and nothing asserted a size budget. This is that budget, here,
