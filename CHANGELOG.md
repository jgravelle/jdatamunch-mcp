...
a `ToolSearch`-style lookup fetches them. That is 39 bare strings here,
...
rather than a feature summary. 897 characters against a 1,000 cap; nothing
proves a longer string survives un-truncated.

### Fixed - `serverInfo` named the SDK's version, not ours

`Server("jdatamunch-mcp")` was constructed with no `version=`, so the SDK filled the
field with its own package version. Every host that displays a server version
displayed the mcp package number instead of 1.31.8.
...
## [1.31.8] - 2026-08-20 - The licence became something you can point at
...
⚠ **PyPI metadata is immutable per version, so 1.31.8 is where this starts.**
Every version up to and including 1.31.7 keeps empty licence fields, permanently.
